...
class HrPayslip(models.Model):
    ...
    def get_housing_allowance_inputs(self, contract_ids, date_from, date_to):
        ...
        if contract_ids[0].housing_allowance > 0:
            ...
            is_ok = False
            if (contract.house_allowance_months > 0 and contract.housing_allowance > 0) and (
            not self.input_line_ids.filtered(lambda i: i.housing_contract_id.id == contract.id)):
                if contract.house_allowance_last_date:
                    current_date = contract.house_allowance_last_date + relativedelta(
                        months=contract.house_allowance_months)
    ...
    def action_payslip_done(self):
        ...
        if self.employee_id.employee_type != 'freelance':
