...
class SalonBooking(models.Model):
    ...
    def action_conform(self):
        self.state = 'conform'
        xml_id = 'pways_salon_and_spa_management.mail_template_salon_conformed'
        template_id = self.env['ir.model.data']._xmlid_lookup(xml_id)
        if template_id:
            template_id = template_id[1]  # Get the template ID
