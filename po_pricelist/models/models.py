...
class PurchaseOrderLine(models.Model):
    ...
    def onchange_method(self):
        if self.product_id.id and self.order_id.partner_id and self.order_id.destination_id:
            pricelist = self.env['product.supplierinfo'].search([('destination_id', '=', self.order_id.destination_id.id),('product_tmpl_id', '=', self.product_template_id.id), ('partner_id', '=', self.order_id.partner_id.id)]).filtered(lambda d: d.date_start != False).sorted(
